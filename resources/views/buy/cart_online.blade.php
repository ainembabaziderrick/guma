    ...
    @if($cart && count($cart) > 0)
        <div class="table-responsive">
            ...
            <div class="text-end mt-3">
                <a href="{{ url('/checkout/online') }}" class="btn btn-success px-4">Proceed to Checkout</a>
            </div>
        </div>
    @else
...
$(document).ready(function(){
    ...
    // Update quantity in online cart
    $(".quantity").change(function (e) {
        e.preventDefault();
        var ele = $(this);

        $.ajax({
            url: '{{ route('update.cart.online') }}',
            method: "post",
            data: {
                _token: '{{ csrf_token() }}',
                id: ele.attr("data-id"),
    ...
    $(".remove-from-cart").click(function (e) {
        ...
        if(confirm("Are you sure you want to remove this product?")) {
            $.ajax({
                url: '{{ route('remove.from.cart.online') }}',
                method: "post",
                data: {
                    _token: '{{ csrf_token() }}',
                    id: ele.attr("data-id")
