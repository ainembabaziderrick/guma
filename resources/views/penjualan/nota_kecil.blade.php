...
<body onload="window.print()">
    ...
    <br>
    <div>
        <p style="float: left;">{{ date('d-m-Y H:i:s', strtotime(now()->setTimezone('Africa/Kampala'))) }}</p>
        <!-- <p style="float: right;">{{ strtoupper(auth()->user()->name) }}</p> -->
    </div>
    <div class="clear-both" style="clear: both;"></div>
    <p>No: {{ tambah_nol_didepan($penjualan->id_penjualan, 10) }}</p>
    <p>Customer: {{ $penjualan->member->nama ?? 'Unknown Customer' }}</p>
    ...
    <p class="text-center">Printed by: {{ strtoupper(auth()->user()->name) }}</p>
