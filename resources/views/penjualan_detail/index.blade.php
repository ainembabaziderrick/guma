...
<script>
    ...
    const availableStock = parseFloat($(this).data('available-stock')); // <- This line is key
    const jumlah = parseFloat($(this).val());

    if (jumlah < 0.1) {
        $(this).val(0.1);
        alert('The number cannot be less than 0.1');
        return;
    }

    if (jumlah > availableStock) {
        $(this).val(availableStock);
        alert('The quantity exceeds the available stock. Maximum allowed: ' + availableStock);
        return;
    }
    ...
    $.ajax({
        url: '/get-product-stock/' + id, // Route to check the product stock
        method: 'GET',
        success: function(data) {
            const availableStock = data.stock;
            
            if (availableStock < 1) {
                alert('Error: Stock for product ' + kode + ' is less than 1. Transaction cannot proceed.');
                return; // Prevent transaction continuation
            } else if (availableStock < 20) {
                alert('Warning: Stock for product ' + kode + ' is low (less than 2). Proceeding with transaction.');
            }
            
            // Store available stock in a hidden field for validation
            $('#id_produk').val(id);
            $('#kode_produk').val(kode);
            $('#id_produk').data('availableStock', availableStock); // Store stock in element's data
            hideProduk();
            tambahProduk();
        }
    });
