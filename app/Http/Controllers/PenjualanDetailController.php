...
class PenjualanDetailController extends Controller
{
    ...
    public function data($id)
    {
        ...
        foreach ($detail as $item) {
            ...
            $row['harga_jual']  = 'Ugx '. $item->harga_jual;
            $row['jumlah'] = '<input type="number" step="0.1" min="0" class="form-control input-sm quantity" 
            data-id="'. $item->id_penjualan_detail .'" 
            data-available-stock="'. $item->produk->stok .'" 
            value="'. number_format($item->jumlah, 1, '.', '') .'">';
        


            $row['diskon']      = $item->diskon . '%';
            $row['subtotal']    = 'Ugx '. $item->subtotal;
