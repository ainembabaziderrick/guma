<?php

namespace App\Http\Controllers;

use App\Models\Penjualan;
use App\Models\PenjualanDetail;
use App\Models\Produk;
use App\Models\Setting;
use App\Models\Debts;
use App\Models\Debtors;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Auth; 
use Carbon\Carbon; // Ensure Carbon is imported


use PDF;

class PenjualanController extends Controller
{
    public function index()
    {
        return view('penjualan.index');
    }

    public function data()
    {
        $penjualan = Penjualan::with('member')->orderBy('id_penjualan', 'desc')->get();
    
        return datatables()
            ->of($penjualan)
            ->addIndexColumn()
            ->addColumn('total_item', function ($penjualan) {
                return $penjualan->total_item;
            })
            ->addColumn('total_harga', function ($penjualan) {
                return 'Ugx ' . number_format($penjualan->total_harga, 2);
            })
            ->addColumn('bayar', function ($penjualan) {
                return 'Ugx ' . number_format($penjualan->bayar, 2);
            })
            ->addColumn('tanggal', function ($penjualan) {
                return tanggal_indonesia($penjualan->created_at, false);
            })
            ->addColumn('kode_member', function ($penjualan) {
                $member = $penjualan->member->kode_member ?? '';
                return '<span class="label label-success">' . $member . '</span>';
            })
            ->editColumn('diskon', function ($penjualan) {
                return $penjualan->diskon . '%';
            })
            ->editColumn('kasir', function ($penjualan) {
                return $penjualan->user->name ?? '';
            })
            ->addColumn('balance', function ($penjualan) {
                $balance = $penjualan->total_harga - $penjualan->diterima;
                return 'Ugx ' . number_format($balance);
            })
            ->addColumn('aksi', function ($penjualan) {
                return '
                <div class="btn-group">
                    <button onclick="showDetail(`'. route('penjualan.show', $penjualan->id_penjualan) .'`, `'. addslashes($penjualan->member->nama ?? 'Unknown') .'`)" 
                        class="btn btn-xs btn-primary btn-flat">
                        <i class="fa fa-eye"></i>
                    </button>
                    <button onclick="deleteData(`'. route('penjualan.destroy', $penjualan->id_penjualan) .'`)" 
                        class="btn btn-xs btn-danger btn-flat">
                        <i class="fa fa-trash"></i>
                    </button>
                </div>
                ';
            })
            ->rawColumns(['kode_member', 'aksi'])
            ->make(true);
    }
    
    // visit "codeastro" for more projects!
    public function create()
    {
        $penjualan = new Penjualan();
        $penjualan->id_member = null;
        $penjualan->total_item = 0;
        $penjualan->total_harga = 0;
        $penjualan->diskon = 0;
        $penjualan->bayar = 0;
        $penjualan->diterima = 0;
        $penjualan->id_user = auth()->id();
        $penjualan->save();

        session(['id_penjualan' => $penjualan->id_penjualan]);
        return redirect()->route('transaksi.index');
    }

    public function store(Request $request)
{
    $penjualan = Penjualan::findOrFail($request->id_penjualan);

    $subtotal = $request->total;
    $bayar = $request->bayar;
    $diterima = $request->diterima;

    $user = auth()->user();

    if ($user->level == 2 && ($bayar > $diterima || $bayar < $subtotal)) {
        return redirect()->back()->with('error', 'Insufficient payment. Transaction cannot be completed.');
    }

    // 🔴 CHECK EXPIRY BEFORE COMPLETING SALE
    $detail = PenjualanDetail::where('id_penjualan', $penjualan->id_penjualan)->get();

    foreach ($detail as $item) {
        $produk = Produk::find($item->id_produk);

        if ($produk && $produk->expiry_date) {
            $expiryDate = Carbon::parse($produk->expiry_date);
            $today = Carbon::now();
            $daysRemaining = $today->diffInDays($expiryDate, false);

            // If expiry is within 10 days
            if ($daysRemaining <= 2) {
                return redirect()->back()->with('error',
                    '⚠ Product "' . $produk->nama_produk . '" is expiring in '
                    . $daysRemaining . ' day(s). Transaction cannot be completed.'
                );
            }
        }
    }

    // ✅ If no expiry problem, continue saving
    $penjualan->id_member = $request->id_member;
    $penjualan->total_item = $request->total_item;
    $penjualan->total_harga = $subtotal;
    $penjualan->diskon = $request->diskon;
    $penjualan->bayar = $bayar;
    $penjualan->diterima = $diterima;
    $penjualan->update();

    foreach ($detail as $item) {
        $item->diskon = $request->diskon;
        $item->update();

        $produk = Produk::find($item->id_produk);
        $produk->stok -= $item->jumlah;
        $produk->update();
    }

    return redirect()->route('transaksi.selesai');
}
    
    public function show($id)
    {
        $detail = PenjualanDetail::with('produk')->where('id_penjualan', $id)->get();

        return datatables()
            ->of($detail)
            ->addIndexColumn()
            ->addColumn('kode_produk', function ($detail) {
                return '<span class="label label-success">'. $detail->produk->kode_produk .'</span>';
            })
            ->addColumn('nama_produk', function ($detail) {
                return $detail->produk->nama_produk;
            })
            ->addColumn('harga_jual', function ($detail) {
                return 'Ugx '. $detail->harga_jual;
            })
            ->addColumn('jumlah', function ($detail) {
                return $detail->jumlah;
            })
            ->addColumn('subtotal', function ($detail) {
                return 'Ugx  '. $detail->subtotal;
            })
            ->rawColumns(['kode_produk'])
            ->make(true);
    }
    // visit "codeastro" for more projects!
    public function destroy($id)
    {
        $penjualan = Penjualan::find($id);
        $detail    = PenjualanDetail::where('id_penjualan', $penjualan->id_penjualan)->get();
        foreach ($detail as $item) {
            $produk = Produk::find($item->id_produk);
            if ($produk) {
                $produk->stok += $item->jumlah;
                $produk->update();
            }

            $item->delete();
        }

        $penjualan->delete();

        return response(null, 204);
    }

    public function selesai()
    {
        $setting = Setting::first();

        return view('penjualan.selesai', compact('setting'));
    }

    public function notaKecil()
    {
        $setting = Setting::first();
        $penjualan = Penjualan::find(session('id_penjualan'));
        if (! $penjualan) {
            abort(404);
        }
        $detail = PenjualanDetail::with('produk')
            ->where('id_penjualan', session('id_penjualan'))
            ->get();
        
        return view('penjualan.nota_kecil', compact('setting', 'penjualan', 'detail'));
    }

    public function notaBesar()
    {
        $setting = Setting::first();
        $penjualan = Penjualan::find(session('id_penjualan'));
        if (! $penjualan) {
            abort(404);
        }
        $detail = PenjualanDetail::with('produk')
            ->where('id_penjualan', session('id_penjualan'))
            ->get();

        $pdf = PDF::loadView('penjualan.nota_besar', compact('setting', 'penjualan', 'detail'));
        $pdf->setPaper(0,0,609,440, 'potrait');
        return $pdf->stream('Transaction-'. date('Y-m-d-his') .'.pdf');
    }

    public function Debts(){
        $data['getRecord'] = Debts::get();
        return view('debts.list', $data);
    }

    public function add_debts(Request $request){
        return view('debts.add');
    }

    
    public function insert_add_debts(Request $request)
    {
        $save = new Debts;
        $save->name = trim($request->name);
        $save->description = trim($request->description);
        $save->amount = trim($request->amount);
        $save->date = trim($request->date);
        
        // Set created_at and updated_at to the current time in Africa/Kampala timezone
        $save->created_at = Carbon::now('Africa/Kampala');
        $save->updated_at = Carbon::now('Africa/Kampala');
        
        $save->save();
    
        return redirect('/debts')->with('success', 'Debt successfully created');
    }
    

    public function Deletedebts($id){
        $delete = Debts::find($id)->Delete();
        return Redirect()->back()->with('success','Debts Paid succcessfully');
     }

     public function Debtors(){
        $getRecord = Debtors::where('amount', '>', 0)->get();

        return view('debtors.list', compact('getRecord'));
       
    }

    public function add_debtors(Request $request){
        return view('debtors.add');
    }

    public function insert_add_debtors(Request $request){
        $save = new Debtors;
        $save->name = trim($request->name);
        $save->description = trim($request->description);
        $save->amount = trim($request->amount);
        $save->date = trim($request->date);
         $save->status = 'approved';
        
        $save->save();

        return redirect('/debtor')->with('success', 'Debtors successfully created');
        
    }

    public function Deletedebtors($id){
        $delete = Debtors::find($id)->Delete();
        return Redirect()->back()->with('success','Debtor Cleared succcessfully');
     }

    

public function checkLowStock()
{
    $lowStockProducts = Produk::where('stok', '<', 2)->get();

    if ($lowStockProducts->isNotEmpty()) {
        foreach ($lowStockProducts as $product) {
            // Notify the user (or admin)
            Notification::route('mail', 'admin@example.com') // Replace with your admin email
                ->notify(new LowStockNotification($product));
        }
    }
}
public function getProductStock($id)
{
    $product = Produk::findOrFail($id);
    return response()->json(['stock' => $product->stok]);
}


}
