...
class PembelianDetailController extends Controller
{
    ...
    public function data($id)
    {
        ...
        foreach ($detail as $item) {
            ...
            $row['harga_beli']  = 'Ugx '. format_uang($item->harga_beli);
            $row['jumlah']      = '<input type="number" step="0.1" class="form-control input-sm quantity" data-id="'. $item->id_pembelian_detail .'" value="'. $item->jumlah .'">';
            $row['subtotal']    = 'Ugx '. format_uang($item->subtotal);
